  ...
  routes.post("/save-tactic", async (c) => {
    try {
      ...
      const values = players.map((p: any) => ({
        ...
        isCalledUp: p.isCalledUp ?? !p.isOnPitch,
      }));
      if (keys.length > 0) {
  ...
  routes.post("/save-snapshot", async (c) => {
    try {
      ...
      const capRaw = body.captainPlayerId;
      const captainPlayerId =
        capRaw != null && String(capRaw).trim() !== "" ? String(capRaw) : null;
  ...
  routes.put("/saved-tactic/:id", async (c) => {
    try {
      ...
      const savedAt = new Date().toISOString();
      const prevCap = (prev as { captainPlayerId?: string | null }).captainPlayerId;
      const capFromBody = body.captainPlayerId;
      const captainPlayerId = Object.prototype.hasOwnProperty.call(body, "captainPlayerId")
        ? capFromBody != null && String(capFromBody).trim() !== ""
          ? String(capFromBody)
          : null
        : prevCap != null && String(prevCap).trim() !== ""
          ? String(prevCap)
          : null;
