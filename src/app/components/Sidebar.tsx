...
import { Camera, Trash2, User, UserPlus, ArrowLeftRight, ChevronDown, ChevronUp, LogIn, Star, X, Upload, Loader2 } from 'lucide-react';
        ...
        <div className="space-y-1.5">
          ...
          {pitchPlayers.map(p => (
            <PlayerRow key={p.id} player={p} isSelected={selectedPlayerId === p.id} isDark={isDark}
              ...
              onClick={() => { if (!subMode) onSelectPlayer(p.id); }}
              actionBtn={<button onClick={e => { e.stopPropagation(); setSubMode(p.id); }} className="p-1.5 rounded-md bg-amber-500/10 hover:bg-amber-500/30 text-amber-400"><ArrowLeftRight className="w-3.5 h-3.5" /></button>} />
          ))}
        </div>
            ...
            {showBench && sortedBench.map(p => {
              ...
              return (
                <motion.div key={p.id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                  <div onClick={() => subMode ? handleSubSelect(p.id) : onSelectPlayer(p.id)}
