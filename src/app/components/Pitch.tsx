...
const PlayerToken: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div
        data-clean-capture
        className={`relative flex flex-col items-center transition-all duration-200 ${isSelected ? 'scale-110' : ''}`}
      >
        {isCaptain && (
          <span
            data-clean-capture
            className="absolute -left-1.5 -top-2 z-30 flex h-5 w-5 items-center justify-center rounded-full border-2 border-black/50 bg-amber-400 text-[8px] font-black text-black shadow-md md:h-6 md:w-6 md:text-[9px]"
            title="Capitán"
          >
            C
          </span>
        ...
        <div
          data-clean-capture
          title={player.name}
          className={`mt-2 max-w-[min(100vw,210px)] truncate rounded-xl px-2.5 py-1 text-center text-[9px] font-black leading-tight shadow-[0_8px_18px_rgba(0,0,0,.24)] backdrop-blur-md md:text-[11px] md:leading-tight ${
            isSelected ? 'bg-yellow-400 text-black' : 'bg-black/80 text-white ring-1 ring-white/15'
          } pointer-events-none`}
        >
          {shortName}
        </div>
